...
SpiralSTC::~SpiralSTC()
{
  RCLCPP_INFO(
    ...
    "Destroying plugin %s of type FullCoveragePathPlanner",
    name_.c_str());
}
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  if (!initialized_) {
    ...
    // Currently this plugin does not use the costmap, instead request a map
    // from a server (will change in the future)
    ...
    grid_pub =
      node_->create_publisher<visualization_msgs::msg::Marker>("grid", 0);
    spirals_pub =
      node_->create_publisher<visualization_msgs::msg::Marker>("spirals", 0);

    // Define vehicle width
    double vehicle_width_default = 1.1;
    declare_parameter_if_not_declared(
      node_, name_ + ".vehicle_width",
      rclcpp::ParameterValue(vehicle_width_default));
    ...
    declare_parameter_if_not_declared(
      node_, name_ + ".division_factor",
      rclcpp::ParameterValue(division_factor_default));
    node_->get_parameter(name_ + ".division_factor", division_factor_);
    // Define numer of intermediate footprints used to compute the swept path of
    // a manoeuvre
    int manoeuvre_resolution_default = 100;
    declare_parameter_if_not_declared(
      node_, name_ + ".manoeuvre_resolution",
      rclcpp::ParameterValue(manoeuvre_resolution_default));
    node_->get_parameter(
      name_ + ".manoeuvre_resolution",
      manoeuvre_resolution_);
    // Define maximum allowable overlapping grids between a turning manoeuvre
    // and already visited grids
    int max_overlap_turn_default = 2;
    declare_parameter_if_not_declared(
      node_, name_ + ".max_overlap_turn",
      rclcpp::ParameterValue(max_overlap_turn_default));
    node_->get_parameter(name_ + ".max_overlap_turn", max_overlap_turn_);
    // Define maximum allowable overlapping grids between a forward manoeuvre
    // and already visited grids
    int max_overlap_forward_default = 0;
    declare_parameter_if_not_declared(
      node_, name_ + ".max_overlap_forward",
      rclcpp::ParameterValue(max_overlap_forward_default));
    node_->get_parameter(name_ + ".max_overlap_forward", max_overlap_forward_);
    bool enable_path_smoothing_default = false;
    declare_parameter_if_not_declared(
      node_, name_ + ".enable_path_smoothing",
      rclcpp::ParameterValue(enable_path_smoothing_default));
    node_->get_parameter(
      name_ + ".enable_path_smoothing",
      enable_path_smoothing_);
    double max_smoothing_resolution_default = 0.05;
    declare_parameter_if_not_declared(
      node_, name_ + ".max_smoothing_resolution",
      rclcpp::ParameterValue(max_smoothing_resolution_default));
    node_->get_parameter(
      name_ + ".max_smoothing_resolution",
      max_smoothing_resolution_);
...
void SpiralSTC::activate()
{
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Activating plugin %s of type FullCoveragePathPlanner",
    name_.c_str());
...
void SpiralSTC::deactivate()
{
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Deactivating plugin %s of type FullCoveragePathPlanner",
    name_.c_str());
...
void SpiralSTC::cleanup()
{
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Cleaning up plugin %s of type FullCoveragePathPlanner",
    name_.c_str());
  plan_pub_.reset();
  grid_pub.reset();
  spirals_pub.reset();
...
SpiralSTC::spiral(
  ...
  std::list<gridNode_t> & init, double & yaw_start,
  std::vector<std::vector<bool>> & visited)
{
  spiral_cpp_metrics_type stored_spiral_cpp_metrics;
  std::list<gridNode_t> path_nodes(
    init);   // Copy incoming init list to path_nodes
  std::list<gridNode_t>::iterator it =
    --(path_nodes.end());      // Create iterator and let it point to the last
                               // element of end
  ...
  if (!computeFootprintCells(
      ...
      yaw_current, "tool", init_cells))
  {
    RCLCPP_ERROR(
      rclcpp::get_logger("FullCoveragePathPlanner"),
      "Starting footprint seems to be out of bounds!");
  }
  ...
  // Start collecting visited cells to remove if added nodes turn out to be
  // unsafe
  unsafe_visited_cells.insert(
    unsafe_visited_cells.end(), init_cells.begin(),
    init_cells.end());
  ...
  while (!done) {
    ...
    // Save the current node as the most recent safe node if it is safe to turn
    // around (left, right or both)
    bool current_node_is_safe = false;
    if (transformRelativeManoeuvre(
        x_current, y_current, yaw_current,
        vehicle_turn_around_left_rel, man_cells))
    {
      if (checkManoeuvreCollision(man_cells, grid)) {
        current_node_is_safe = true;
        spiral_has_safe_node = true;
        stored_spiral_cpp_metrics =
          spiral_cpp_metrics_;   // Store the current state of metrics in case
                                 // we recover back to this node
        unsafe_visited_cells
        .clear();     // All visited cells up until this point are safe
        it_safe_node =
          --(path_nodes
          .end());          // Can turn around left without obstacles or going
                            // out of bounds, so remember as a safe node
      }
    }
    if (transformRelativeManoeuvre(
        x_current, y_current, yaw_current,
        vehicle_turn_around_right_rel, man_cells))
    {
      if (checkManoeuvreCollision(man_cells, grid)) {
        current_node_is_safe = true;
        spiral_has_safe_node = true;
        ...
        it_safe_node =
          --(path_nodes
          .end());          // Can turn around right without obstacles or going
                            // out of bounds, so remember as a safe node
      }
    }

    // Control in which direction (out of the 3) the spiral will start looking
    // to explore after restoring to a safe node
    int i_start = 0;
    if (restored) {
      // If the planner just restored to a recent safe node, configure variables
      // so that it will try exploring the next untested direction
      i_start =
        explored_dir + 1;   // Start looking in the first unexplored direction
    ...
    // Loop over the three possible directions: left, forward, right (directions
    // taken before initial counter-clockwise turn)
    done = true;  // This condition might change in the loop below
    int collision_manoeuvres =
      0;   // Counting how many (of the three) manoeuvres result in a collision
    for (int i = i_start; i < 3; ++i) {
      ...
      RCLCPP_INFO(
        rclcpp::get_logger("FullCoveragePathPlanner"),
        "Manoeuvre from (x=%d, y=%d, yaw=%f) to (x=%d, y=%d, yaw=%f)",
        x_current, y_current, yaw_current, x_next, y_next, yaw_next);

      // Changing parameter indicating the maximum overlap for a given manoeuvre
      int max_overlap;
      // This condition might change in the loop below,
      // indicating a manoeuvre is free
      bool man_is_free = true;

      // Apply a rotation to the relative manoeuvre cells to convert from robot
      // frame to world frame
      if (i == 0) {
        // Transform standard relative left turn manoeuvre
        if (!transformRelativeManoeuvre(
            x_current, y_current, yaw_current,
            vehicle_left_turn_rel, man_cells) ||
          !transformRelativeManoeuvre(
            x_current, y_current, yaw_current,
            tool_left_turn_rel, visited_cells))
        {
          man_is_free = false;
          RCLCPP_INFO(
            rclcpp::get_logger("FullCoveragePathPlanner"),
            "  --> out of bounds, looking in other directions...");
        }
        max_overlap = max_overlap_turn_;
      } else if (i == 1) {
        // Transform standard relative forward manoeuvre
        if (!transformRelativeManoeuvre(
            x_current, y_current, yaw_current,
            vehicle_forward_rel, man_cells) ||
          !transformRelativeManoeuvre(
            x_current, y_current, yaw_current,
            tool_forward_rel, visited_cells))
        {
          man_is_free = false;
          RCLCPP_INFO(
            rclcpp::get_logger("FullCoveragePathPlanner"),
        ...
        // Transform standard relative right turn manoeuvre
        if (!transformRelativeManoeuvre(
            x_current, y_current, yaw_current,
            vehicle_right_turn_rel, man_cells) ||
          !transformRelativeManoeuvre(
            x_current, y_current, yaw_current,
            tool_right_turn_rel, visited_cells))
        {
          man_is_free = false;
          RCLCPP_INFO(
            rclcpp::get_logger("FullCoveragePathPlanner"),
      ...
      if (!checkManoeuvreCollision(man_cells, grid)) {
        // Keep track of how many of the tested directions are rejected
        // due to a possible collision
        collision_manoeuvres += 1;
        man_is_free = false;
        RCLCPP_INFO(
          rclcpp::get_logger("FullCoveragePathPlanner"),
          "  --> causing a collision, looking in other directions");
      }
      ...
      if (spiral_has_safe_node) {
        ...
        RCLCPP_INFO(
          rclcpp::get_logger("FullCoveragePathPlanner"),
          "  --> the most recent safe node is (x=%d,y=%d)",
          safe_node.pos.x, safe_node.pos.y);
      }

      if (collision_manoeuvres == 3) {
        // All possible manoeuvres would result in a collision
        RCLCPP_INFO(
          rclcpp::get_logger("FullCoveragePathPlanner"),
          "!!!!!!!!!!!! All 3 directions lead to a collision !!!!!!!!!!!!");
        if (spiral_has_safe_node) {
          std::list<gridNode_t>::iterator it_start_unsafe_path =
            ++(it_safe_node);
          RCLCPP_INFO(
            rclcpp::get_logger("FullCoveragePathPlanner"),
            "  --> length before removing unsafe nodes: %lu",
            path_nodes.size());
          path_nodes.erase(it_start_unsafe_path, path_nodes.end());
          RCLCPP_INFO(
            rclcpp::get_logger("FullCoveragePathPlanner"),
            "  --> length after removing unsafe nodes: %lu",
            path_nodes.size());
          ...
          for (const auto unsafe_visited_cell : unsafe_visited_cells) {
            ...
            visited_copy[unsafe_visited_cell.y][unsafe_visited_cell.x] =
              eNodeOpen;
          }

          // Restore footprints of some of the final nodes in the remaining
          // path, as they were possibly affected by the above operation
          for (int i = 0; i < division_factor_; i++) {
            if (static_cast<size_t>(i) < path_nodes.size()) {
              ...
              computeFootprintCells(
                this_node.pos.x, this_node.pos.y, yaw,
                "tool", cells);
          ...
          RCLCPP_INFO(
            rclcpp::get_logger("FullCoveragePathPlanner"),
            "!!!!!!!!!!!! Spiral does not have safe node but needs "
            "to be restored !!!!!!!!!!!!");
          ...
          RCLCPP_INFO(
            rclcpp::get_logger("FullCoveragePathPlanner"),
            "!!!!!!!!!!!! Restored to start of the spiral, no safe "
            "nodes in there... !!!!!!!!!!!!");
          break;
        }
      }
      ...
      RCLCPP_INFO(
        rclcpp::get_logger("FullCoveragePathPlanner"),
        "  --> manoeuvre size=%lu (& %lu for tool) of which %d are "
        "overlapping",
        man_cells.size(), visited_cells.size(), overlap);

      // When all conditions are met, add the point to path_nodes and mark the
      // covered cells as visited
      if (man_is_free && overlap <= max_overlap) {
        ...
        it = --(path_nodes.end());
        if (current_node_is_safe) { // Store the direction of exploration made
                                    // from the safe node
          explored_dir =
            i;   // The for loop iterator i that represents the chosen direction
          stored_spiral_cpp_metrics = spiral_cpp_metrics_;
        }
        spiral_cpp_metrics_.visited_counter += visited_cells.size();
        spiral_cpp_metrics_.multiple_pass_counter += overlap;
        unsafe_visited_cells.insert(
          unsafe_visited_cells.end(), visited_cells.begin(),
          visited_cells
          .end());       // Will be cleared once a node is considered safe
        for (const auto visited_cell : visited_cells) {
          visited[visited_cell.y][visited_cell.x] = eNodeVisited;
 ...
 * @return whether last elem. of first has a larger heuristic value than last
 * elem of second
 */
bool sort_gridNodePath_heuristic_desc(
  const std::vector<gridNode_t> & first,
  const std::vector<gridNode_t> & second)
{
...
bool SpiralSTC::planAStarToOpenSpace(
  std::vector<std::vector<bool>> const & grid,
  gridNode_t init, double & yaw_init,
  int cost,
  std::vector<std::vector<bool>> & visited,
  std::list<Point_t> const & open_space,
  std::list<gridNode_t> & path_nodes)
{
  int dx, dy, dx_prev, n_rows = grid.size(), n_cols = grid[0].size();

  std::vector<std::vector<bool>> closed(n_rows,
    std::vector<bool>(n_cols, eNodeOpen));
  // All nodes in the closed list are currently still open

  closed[init.pos.y][init.pos.x] =
    eNodeVisited;   // Of course we have visited the current/initial location

#ifdef DEBUG_PLOT
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "A*: Marked init gridNode_t((%d, %d), %d, %d) as eNodeVisited (true)",
    init.pos.x, init.pos.y, init.cost, init.he);
#endif

  std::vector<std::vector<gridNode_t>> open(
    1, std::vector<gridNode_t>(1, init));   // open is a *vector* of paths

  while (true) { // Keep searching until either a path is found or no path can
                 // be found

#ifdef DEBUG_PLOT
    RCLCPP_INFO(
      rclcpp::get_logger("FullCoveragePathPlanner"),
      "A*: open.size() = %lu", open.size());
#endif

    if (open.size() == 0) { // If there are no open paths, there's no place to
                            // go and we must resign
      // Empty end_node list and add init as only element
      path_nodes.erase(path_nodes.begin(), --(path_nodes.end()));
      path_nodes.push_back(init);
      // We resign, cannot find a path
      return true;
    } else {
      // Sort elements from high to low (because
      // sort_gridNodePath_heuristic_desc uses a > b)
      // Sort bases on heuristic costs
      std::sort(
        open.begin(), open.end(),
        sort_gridNodePath_heuristic_desc);
      // Get the *path* with currently the lowest heuristic cost
      std::vector<gridNode_t> nn = open.back();
      // The last element is no longer open because we use it
      // here, so remove from open list
      open.pop_back();

#ifdef DEBUG_PLOT
      RCLCPP_INFO(
        rclcpp::get_logger("FullCoveragePathPlanner"),
        "A*: Check out path from (%d, %d) to (%d, %d) of length %lu",
        nn.front().pos.x, nn.front().pos.y, nn.back().pos.x,
        nn.back().pos.y, nn.size());
#endif

      // Does the path nn end in open space?
      if (visited[nn.back().pos.y][nn.back().pos.x] == eNodeOpen) {
        // If so, we found a path to open space
        // Copy the path nn to path_nodes so we can report that path (to get to
        // open space)
        std::vector<gridNode_t>::iterator iter;
        for (iter = nn.begin(); iter != nn.end(); ++iter) {
          path_nodes.push_back((*iter));
        }
        // We do not resign, we found a path
        return false;
      } else {
        double yaw_current;
        if (nn.size() > 1) {
          // Create iterator for gridNode_t list and let it point to the last
          // element of nn
        ...
        for (int i = 0; i < 4; ++i) {
          ...
          RCLCPP_INFO(
            rclcpp::get_logger("FullCoveragePathPlanner"),
            "A*: Look around in direction %d at p_next=(%d, %d)", i,
            p_next.x, p_next.y);
#endif
          ...
          // TODO(AronTiemessen): Add the overlap condition here instead of
          // starting over with A* all the time in spiral_stc?? If the new node
          // (a neighbor of the end of the path nn) is open, append it to
          // new_path ( = nn) and add that to the open-list of paths. Because of
          // the pop_back on open, what happens is that the path is temporarily
          // 'checked out', modified here, and then added back (if the condition
          // above and below holds)
            ...
            checkManoeuvreCollision(man_cells, grid) && !out_of_bounds)
          {
            ...
            RCLCPP_INFO(
              rclcpp::get_logger("FullCoveragePathPlanner"),
              "A*: p_next=(%d, %d) is OPEN", p_next.x, p_next.y);
#endif

            std::vector<gridNode_t> new_path = nn;
            // The heuristic has to be designed to prefer a CCW
            ...
            gridNode_t new_node = {
              ...
              cost + nn.back().cost +
              distanceToClosestPoint(p_next, open_space) +
              i       // Heuristic (+i so CCW turns are cheaper)
            };
            ...
            RCLCPP_INFO(
              rclcpp::get_logger("FullCoveragePathPlanner"),
              "A*: Marked new_node gridNode_t((%d, %d), %d, %d) as "
              "eNodeVisited (true)",
              new_node.pos.x, new_node.pos.y, new_node.cost,
              new_node.he);
            RCLCPP_INFO(
              rclcpp::get_logger("FullCoveragePathPlanner"),
              "A*: Add path from (%d, %d) to (%d, %d) of length %lu to open",
              new_path.front().pos.x, new_path.front().pos.y,
              new_path.back().pos.x, new_path.back().pos.y, new_path.size());
#endif
            ...
            RCLCPP_INFO(
              rclcpp::get_logger("FullCoveragePathPlanner"),
              "A*: p_next=(%d, %d) is not open: closed or collision",
              p_next.x, p_next.y);
          }
#endif
...
SpiralSTC::spiral_stc(
  std::vector<std::vector<bool>> const & grid, Point_t & init,
  double & yaw_start)
{
  ...
  RCLCPP_INFO(
    ...
    init_x, init_y, yaw_start);
  path_nodes =
    spiral(grid, path_nodes, yaw_start, visited);   // First spiral fill

  visualizeSpiral(path_nodes, "first_spiral", 0.2, 0.5, 0.0, 0.6, 0.0);
  // Retrieve remaining goal points
  std::list<Point_t> goals = retrieveGoalsFromMap(
    visited, eNodeOpen);
  ...
  while (goals.size() != 0) {
    ...
    //     "@@@@@@@@@ BREAK INSERTED TO ONLY PLAN CERTAIN AMOUNT OF SPIRALS
    ...
    if (path_nodes.size() > 0) {
      ...
      std::advance(it, -2);
      yaw_init = std::atan2(
        path_nodes.back().pos.y - it->pos.y,
        path_nodes.back().pos.x - it->pos.x);
    }

    // Remove all elements from path_nodes list except last element (starting
    // point for A*)
    path_nodes.erase(path_nodes.begin(), --(path_nodes.end()));

    RCLCPP_INFO(
      rclcpp::get_logger("FullCoveragePathPlanner"),
      "!!!!!!!!!!!! Starting an A* path from (x=%d, y=%d, yaw=%f) "
      "!!!!!!!!!!!!",
      path_nodes.back().pos.x, path_nodes.back().pos.y, yaw_init);
    // Plan to closest open Node using A*. Here, `goals` is essentially the map,
    // so we use `goals` to determine the distance from the end of a potential
    // path to the nearest free space
    bool accept_a_star = false;
    bool resign;
    while (!accept_a_star) {
      resign = planAStarToOpenSpace(
        grid, path_nodes.back(), yaw_init, 1,
        visited, goals, path_nodes);
      if (resign) {
        break;
      }
      ...
      accept_a_star = computeFootprintCells(
        x_n, y_n, yaw, "vehicle",
        a_star_end_vehicle_footprint);
      ...
      for (const auto a_star_end_vehicle_cell : a_star_end_vehicle_footprint) {
        if (grid[a_star_end_vehicle_cell.y][a_star_end_vehicle_cell.x] ==
          eNodeVisited)
        {
          accept_a_star = false;
          break;
      ...
      for (const auto a_star_end_tool_cell : a_star_end_tool_footprint) {
        if (visited[a_star_end_tool_cell.y][a_star_end_tool_cell.x] ==
          eNodeVisited)
        {
          visit_count++;
        }
    ...
    // TODO(AronTiemessen): Currently done using static footprints, convert to
    // manoeuvres
    for (std::list<gridNode_t>::iterator it = path_nodes.begin();
      it != path_nodes.end(); ++it)
    {
      ...
      if (it == path_nodes.begin()) {
        ...
        computeFootprintCells(
          current.pos.x, current.pos.y, yaw, "tool",
          visited_cells);
    ...
    // TODO(AronTiemessen): Reset debugging grid
    visited = visited_copy;

    RCLCPP_INFO(
      rclcpp::get_logger("FullCoveragePathPlanner"),
      "--> size of A* path to closest open node is %lu",
      path_nodes.size());

    gridNode_t last_node_a_star =
      path_nodes.back();   // Save the value of the final node of the A star
                           // path to compare to later

    // Spiral fill from current position (added to A* transition path)
    std::list<gridNode_t>::iterator second_to_last_node_it =
      --(path_nodes.end());
    ...
    std::list<gridNode_t> path_nodes_spiral =
      path_nodes;   // Work with a copy so that path_nodes is not affected later
    ...
    for (const auto path_node_spiral : path_nodes_spiral) {
      ...
      {
        // Break if iterator is found that matches last node of A* path
        break;
      } else if (it == path_nodes_spiral.end()) {
        // Break if it is already at last node to prevent it becoming
        // bigger than path_nodes_spiral
        break;
      }
      // Iterate one further to find starting point of new spiral
      ++it;
    }
    ...
    if (path_nodes_spiral.size() > 1) {
      SpiralSTC::visualizeSpiral(
        path_nodes_spiral,
        "spiral" + std::to_string(goals.size() + 1),
        0.2, 0.5, 0.0, 0.6, 0.0);
    }

    goals = retrieveGoalsFromMap(
      visited,
      eNodeOpen);                            // Retrieve remaining goal points
  ...
  std::vector<geometry_msgs::msg::PoseStamped> & plan)
{
  ...
  double grid_size = vehicle_width_ / division_factor_;
  if (!parseGrid(costmap_, grid, grid_size, start, start_point, yaw_start)) {
    RCLCPP_ERROR(
      rclcpp::get_logger("FullCoveragePathPlanner"),
      "Could not parse retrieved grid");
    return false;
  }

  // Create a costmap2d object that represents the parsed grid (needed for
  // built-in functionality)
  planner_grid.resizeMap(
    ceil(costmap_->getSizeInMetersX() / tile_size_),
    ceil(costmap_->getSizeInMetersY() / tile_size_),
    tile_size_, grid_origin_.x, grid_origin_.y);

  // Grid visualization with occupied cells greyed out
  visualizeGridlines();
  visualizeGrid(grid, "grid_cubes", 0.5, 0.0, 0.0, 0.0);

  // Compute the standard manoeuvres (for vehicle and tool) to be reused in the
  // spiral loop later
  ...
  vehicle_right_turn_rel =
    computeRelativeManoeuvreFootprint(
    0, -1, std::atan2(-1, 0), eAnyDirection,
    "vehicle", manoeuvre_resolution_);
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Coverage path planning completed!");

  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Removing repeating waypoints");
  // Remove repeating waypoints (result of transition between spiral and A*)
  std::list<Point_t>::iterator it_prev;
  for (std::list<Point_t>::iterator it = goal_points.end();
    it != goal_points.begin(); --it)
  {
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Removed repeating waypoints");

  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Tracking 180 degree turns");
  // Keep track of the 180 degree turns and save the rotation direction in a
  // list
  it_prev = goal_points.begin();
  double yaw, yaw_prev;
  for (std::list<Point_t>::iterator it = goal_points.begin();
    it != goal_points.end(); ++it)
  {
    if (it == goal_points.begin()) {
      ...
      {
        std::vector<nav2_costmap_2d::MapLocation> man_cells;
        bool turn_is_possible =
          transformRelativeManoeuvre(
          it_prev->x, it_prev->y, yaw_prev,
          vehicle_turn_around_left_rel, man_cells);
        if (turn_is_possible && checkManoeuvreCollision(man_cells, grid)) {
          ...
          turn_is_possible = transformRelativeManoeuvre(
            it_prev->x, it_prev->y, yaw_prev, vehicle_turn_around_right_rel,
            man_cells);
          if (turn_is_possible && checkManoeuvreCollision(man_cells, grid)) {
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Found the 180 degree turns");

  std::vector<geometry_msgs::msg::PoseStamped> coarse_plan;
  parsePointlist2Plan(start, goal_points, coarse_plan);
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Goal points converted to a coarse plan");
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "To be cleaned: %d out of %d cells", to_be_cleaned_cells,
    planner_grid.getSizeInCellsX() * planner_grid.getSizeInCellsY());
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Total visited: %d cells", spiral_cpp_metrics_.visited_counter);
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Total revisited: %d cells",
    spiral_cpp_metrics_.multiple_pass_counter);
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Total accessible cells: %d cells",
    spiral_cpp_metrics_.accessible_counter);
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Final coverage: %f percent",
    (static_cast<double>(spiral_cpp_metrics_.accessible_counter) /
    to_be_cleaned_cells) *
    100);
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
  ...
  // TODO(CesarLopez): Check if global path should be calculated repetitively or
  // just kept (also controlled by planner_frequency parameter in move_base
  // namespace)

  std::vector<nav_msgs::msg::Path> path_vector;
  smoothPlan(
    enable_path_smoothing_, max_smoothing_resolution_, grid_size,
    coarse_plan, path_vector);
  // A path vector is made that divides the path into sections. This can be used
  // in the future to directly send it to a Section Navigator module. For now we
  // convert the plan back to a plan
  convertPathVectorToPlan(path_vector, plan);
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"),
    "Publishing plan!");
  publishPlan(plan);
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"), "Elapsed time: %f",
    elapsed_secs);
  ...
  std::vector<nav2_costmap_2d::MapLocation> & footprint_cells)
{
  ...
  if (part == "vehicle") {
    ...
    // TODO(AronTiemessen): This will eventually be published by the coverage
    // plugin, for now hardcoded
  ...
  for (const auto point : footprint) {
    int map_x, map_y;
    planner_grid.worldToMapNoBounds(
      point.x, point.y, map_x,
      map_y);   // Use worldToMapNoBounds() variant because of bugs in the one
                // that enforces bounds
    if (!checkMapBounds(map_x, map_y, x_max, y_max)) {
      // When the requested footprint falls out of bounds, return false
      return false;
    }
    nav2_costmap_2d::MapLocation map_loc{static_cast<uint>(map_x),
      static_cast<uint>(map_y)};
    footprint_ML.push_back(map_loc);
  }

  // Filter out double corner points that happen due to the grid resolution to
  // prevent problems with convexFillCells() later
  ...
  footprint_inds.erase(
    std::unique(footprint_inds.begin(), footprint_inds.end()),
    footprint_inds.end());
  footprint_ML.clear();
  ...
  if (footprint_ML.size() == 1) {
    RCLCPP_INFO(
      rclcpp::get_logger("FullCoveragePathPlanner"),
      "Footprint consists of only 1 point!");
    footprint_cells = footprint_ML;
  } else if (footprint_ML.size() == 2) {
    RCLCPP_INFO(
      rclcpp::get_logger("FullCoveragePathPlanner"),
      "Footprint only consists of 2 points!");
    footprint_cells = footprint_ML;
  }
...
bool SpiralSTC::computeManoeuvreFootprint(
  int & x_current, int & y_current, double & yaw_current, int & x_next,
  int & y_next, double yaw_next, eRotateDirection direction, std::string part,
  ...
  std::vector<nav2_costmap_2d::MapLocation> & man_cells)
{
  ...
  if (part == "vehicle" &&
    !computeFootprintCells(
      x_current, y_current, yaw_current, "vehicle",
      footprint1_cells))
  {
    return false;
  } else if (part == "tool" &&  // NOLINT
    !computeFootprintCells(
      x_current, y_current, yaw_current, "tool",
      footprint1_cells))
  {
    return false;
  }
  ...
  double yaw_inter;
  for (int i = 1; i <= manoeuvre_resolution - 2;
    i++)      // Substract 2 due to initial and final pose are computed
  {           // seperately
    std::vector<nav2_costmap_2d::MapLocation> cells;
    ...
    if (part == "vehicle" &&
      !computeFootprintCells(
        x_current, y_current, yaw_inter, "vehicle",
        cells))
    {
      return false;
    } else if (part == "tool" &&  // NOLINT
      !computeFootprintCells(
        x_current, y_current, yaw_inter, "tool",
        cells))
    {
      return false;
    }

    intermediate_cells.insert(
      intermediate_cells.end(), cells.begin(),
      cells.end());
  }
  ...
  if (part == "vehicle" &&
    !computeFootprintCells(
      x_next, y_next, yaw_next, "vehicle",
      footprint2_cells))
  {
    return false;
  } else if (part == "tool" &&  // NOLINT
    !computeFootprintCells(
      x_next, y_next, yaw_next, "tool",
      footprint2_cells))
  {
    return false;
  }
  footprint2_cells.insert(
    footprint2_cells.end(), intermediate_cells.begin(),
    intermediate_cells.end());
  ...
  for (const auto footprint1_cell : footprint1_cells) {
    int footprint1_ind =
      planner_grid.getIndex(footprint1_cell.x, footprint1_cell.y);
  ...
  for (const auto footprint2_cell : footprint2_cells) {
    int footprint2_ind =
      planner_grid.getIndex(footprint2_cell.x, footprint2_cell.y);
    bool allow = true;
    // Check if an index of footprint 2 matches with any of the indexes of
    // footprint 1
  ...
  // Get rid of duplicates in the to-be-checked cells and convert back to
  // MapLocations
  ...
  int manoeuvre_resolution)
{
  ...
  double yaw_start =
    0.0;   // Relative manoeuvres are defined with respect to a rotation of 0.0
  ...
  computeManoeuvreFootprint(
    x_start, y_start, yaw_start, x_end, y_end, yaw_end,
    direction, part, manoeuvre_resolution, man_cells);

  // Convert absolute cell locations to relative cell locations for each
  // manoeuvre
  ...
  std::vector<nav2_costmap_2d::MapLocation> & man_cells)
{
  ...
  // Transform (translate and rotate) the relative manoeuvre to the input map
  // location
  man_cells.clear();
...
Point_t SpiralSTC::rotatePoint(
  int poi_x, int poi_y, int & icr_x, int & icr_y,
  double & yaw)
{
  Point_t p;
  ...
  rotated_x_w =
    icr_x_w + (poi_x_w - icr_x_w) * cos(yaw) - (poi_y_w - icr_y_w) * sin(yaw);
  rotated_y_w =
    icr_y_w + (poi_x_w - icr_x_w) * sin(yaw) + (poi_y_w - icr_y_w) * cos(yaw);
...
void SpiralSTC::visualizeGrid(
  std::vector<std::vector<bool>> const & grid,
  std::string name_space, float a, float r, float g,
  float b)
{
  visualization_msgs::msg::Marker grid_cubes =
    cubeMarker(global_frame_.c_str(), name_space, 0, tile_size_, a, r, g, b);
  geometry_msgs::msg::Point p;
  int ix, iy;
...
void SpiralSTC::visualizeSpiral(
  std::list<gridNode_t> & spiral_nodes,
  std::string name_space, float w, float a,
  float r, float g, float b)
{
  visualization_msgs::msg::Marker spiral =
    lineStrip(global_frame_.c_str(), name_space, 0, w, a, r, g, b);
  geometry_msgs::msg::Point p;
  p.z = 0.0;
...
SpiralSTC::cubeMarker(
  std::string frame_id, std::string name_space, int id,
  float size, float a, float r, float g, float b)
{
