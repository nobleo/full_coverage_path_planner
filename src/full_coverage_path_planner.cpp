  ...
  std::vector<geometry_msgs::msg::PoseStamped> & plan)
{
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"), "Received goalpoints with length: %lu",
    goalpoints.size());
  if (goalpoints.size() < 1) {
    ...
    new_goal.pose.position.x =
      (goalpoints.begin()->x) * tile_size_ + grid_origin_.x + tile_size_ * 0.5;
    new_goal.pose.position.y =
      (goalpoints.begin()->y) * tile_size_ + grid_origin_.y + tile_size_ * 0.5;
    ...
    for (it = goalpoints.begin(); it != goalpoints.end(); ++it) {
      ...
      if (do_publish) {
        ...
        RCLCPP_DEBUG(
          rclcpp::get_logger("FullCoveragePathPlanner"),
          "Voila new point: x=%f, y=%f, o=%f,%f,%f,%f", new_goal.pose.position.x,
  ...
  Point_t & scaledStart)
{
  ...
  size_t nodeColl;
  size_t nodeSize =
    dmax(floor(toolRadius / cpp_costmap->getResolution()), 1);  // Size of node in pixels/units
  size_t robotNodeSize =
    dmax(floor(robotRadius / cpp_costmap->getResolution()), 1);  // RobotRadius in pixels/units
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("FullCoveragePathPlanner"), "nRows: %lu nCols: %lu nodeSize: %lu", nRows,
    nCols, nodeSize);
  ...
  for (size_t iy = 0; iy < nRows; iy = iy + nodeSize) {
    ...
    for (size_t ix = 0; ix < nCols; ix = ix + nodeSize) {
      ...
      for (nodeRow = 0;
           (nodeRow < robotNodeSize) && ((iy + nodeRow) < nRows) && (nodeOccupied == false);
           ++nodeRow) {
        for (nodeColl = 0; (nodeColl < robotNodeSize) && ((ix + nodeColl) < nCols); ++nodeColl) {
          int index_grid = dmax(
            (iy + nodeRow - ceil(static_cast<double>(robotNodeSize - nodeSize) / 2.0)) * nCols +
              (ix + nodeColl - ceil(static_cast<double>(robotNodeSize - nodeSize) / 2.0)),
            0);
