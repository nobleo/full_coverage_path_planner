  ...
  std::list<gridNode_t> & pathNodes)
{
  ...
  std::vector<std::vector<gridNode_t> > open1(
    1, std::vector<gridNode_t>(1, init));  // open1 is a *vector* of paths
  ...
  while (true) {
    ...
    if (open1.size() == 0)  // If there are no open paths, there's no place to go and we must resign
    {
      ...
      open1
        .pop_back();  // The last element is no longer open because we use it here, so remove from open list
#ifdef DEBUG_PLOT
      ...
      } else           // Path nn does not lead to open space
      {
        ...
        for (int i = 0; i < 4; ++i) {
          ...
          if (
            p2.x >= 0 && p2.x < nCols && p2.y >= 0 &&
            p2.y < nRows)  // Bounds check, do not sep out of map
          {
            ...
            if (closed[p2.y][p2.x] == eNodeOpen && grid[p2.y][p2.x] == eNodeOpen) {
              ...
              std::cout << "A*: Add path from " << newPath.front().pos << " to "
                        << newPath.back().pos << " of length " << newPath.size() << " to open1"
                        << std::endl;
#endif
